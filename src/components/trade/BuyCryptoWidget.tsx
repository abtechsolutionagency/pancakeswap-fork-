...
const BuyCryptoWidget: React.FC = () => {
  ...
  const FiatWidget = (
    <Body>
      <div style={{ background: 'rgba(31,199,212,0.08)', borderRadius: 16, padding: '14px 16px', marginBottom: 4 }}>
        <Text small color="textSubtle" style={{ marginBottom: 4 }}>
          {mode === 'onramp' ? '💡 Buy crypto with fiat via our secure partner (KYC required)' : '💡 Sell crypto to fiat via our secure partner (KYC required)'}
        </Text>
      </div>
      <Panel>
        <PanelLabel><Text small color="textSubtle">{mode === 'onramp' ? 'You pay' : 'You sell'}</Text></PanelLabel>
        <PanelRow>
          <AmtInput placeholder="0.00" value={fromAmt} onChange={e => setFromAmt(e.target.value)} type="number" />
          {mode === 'onramp' ? (
            <CoinBtn onClick={() => setSelectFrom(true)}>
              <FiatEmoji>{fromFiat.image}</FiatEmoji>
              {fromFiat.ticker.toUpperCase()} ▼
            </CoinBtn>
          ) : (
            <CoinBtn onClick={() => setSelectFrom(true)}>
              <CoinImg src={fromCoin.image} alt={fromCoin.ticker} />
      ...
      <SwitchRow>
        <SwitchBtn onClick={() => setMode(mode === 'onramp' ? 'offramp' : 'onramp')} title="Switch">⇅</SwitchBtn>
      </SwitchRow>

      <Panel>
        <PanelLabel><Text small color="textSubtle">{mode === 'onramp' ? 'You receive' : 'You receive'}</Text></PanelLabel>
        <PanelRow>
          <AmtInput placeholder="0.00" value={toAmt} readOnly style={{ opacity: 0.7 }} />
          {mode === 'offramp' ? (
            <CoinBtn onClick={() => setSelectTo(true)}>
              <FiatEmoji>{toFiat.image}</FiatEmoji>
              {toFiat.ticker.toUpperCase()} ▼
            </CoinBtn>
          ) : (
            <CoinBtn onClick={() => setSelectTo(true)}>
              <CoinImg src={toCoin.image} alt={toCoin.ticker} />
      ...
      <Button
        fullWidth scale="lg"
        onClick={() => window.open(`https://guardarian.com?api_key=${process.env.NEXT_PUBLIC_CHANGENOW_API_KEY}`, '_blank')}
        style={{ marginTop: 4 }}
      >
        {mode === 'onramp' ? '💳 Buy Now' : '💸 Sell Now'}
      </Button>

      <Text small color="textSubtle" textAlign="center">
        Powered by Guardarian via ChangeNow. KYC may be required.
      </Text>
    </Body>
  );
